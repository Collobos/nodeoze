...
namespace strtk
{
   ...
   inline bool type_to_string(const T& t, F func )
   {
      typename details::supported_conversion_from_type<T>::type type;
      return type_to_string_converter_impl(t, type, func);
   }
  ...
  template <typename OutputPredicate>
   ...
   namespace details
   {
      ...
      template <typename T>
      inline bool type_to_string_converter_impl(T value, std::string& result, unsigned_type_tag tag)
      {
		 type_to_string_converter_impl( value, tag, [&]( const char *buf, std::size_t count ) mutable
		 {
	  ...
	  template <typename T, typename F >
      inline bool type_to_string_converter_impl(T value, unsigned_type_tag, F func )
      {
         ...
         func(reinterpret_cast<char*>(itr), (buffer + buffer_size) - itr);
         return true;
      }
      ...
      template <typename T>
      inline bool type_to_string_converter_impl(T valuex, std::string& result, signed_type_tag tag )
	  {
		return type_to_string_converter_impl( valuex, tag, [&]( const char *buf, std::size_t count ) mutable
		{
			result.assign( buf, count );
		} );
	  }
      ...
      template <typename T, typename F>
      inline bool type_to_string_converter_impl(T valuex, signed_type_tag, F func )
      {
         static const std::size_t radix = 10;
         static const std::size_t radix_sqr   = radix * radix;
         static const std::size_t radix_cube  = radix * radix * radix;
